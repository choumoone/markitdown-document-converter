#!/usr/bin/env python3
"""
md_to_html.py — Convert Markdown to polished, self-contained HTML using Pandoc + bundled themes.
...
Part of markitdown-document-converter — Markdown is the source; HTML is a publishable output.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Convert Markdown to polished HTML using Pandoc and bundled themes.",
    )
...
def inline_theme_css(html: str, css_path: Path) -> str:
    css = css_path.read_text(encoding="utf-8")
    css = css.replace("</style>", "<\\/style>")
    link_pattern = re.compile(r'\s*<link\s+rel="stylesheet"\s+href="[^"]*theme\.css"\s*/?>', re.I)
...
def main() -> int:
    ...
    html_text = inline_theme_css(html_text, css_path)
