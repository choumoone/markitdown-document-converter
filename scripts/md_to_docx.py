...
----
# 单文件
python md_to_docx.py article.md
python md_to_docx.py article.md -o article.docx
python md_to_docx.py article.md --images-dir ./images

# 多文件合并成书（按文件名顺序）
python md_to_docx.py ch01.md ch02.md ch03.md -o book.docx

# 完整书模式（带封面 + 目录 + 页眉页脚）
python md_to_docx.py ch*.md --book \\
...
def build_docx(md_files, output, images_dir=None, book_mode=False,
    ...
    if book_mode and title:
        ...
        for idx, md in enumerate(md_files):
            ...
            text = strip_frontmatter(Path(md).read_text(encoding="utf-8-sig"))
    ...
    for idx, md_path in enumerate(md_files):
        ...
        md_text = strip_frontmatter(md_path.read_text(encoding="utf-8-sig"))
